...
class VarRequest(BaseModel):
    ...
    portfolio_id: Optional[str] = Field(default=None, description="Optional portfolio or position identifier.")
...
class SurveillanceRequest(BaseModel):
    ...
    recent_quantities: List[float] = Field(default_factory=list, description="Recent order quantities.")
    recent_notionals: List[float] = Field(default_factory=list)
    instrument_id: Optional[str] = Field(default=None, description="Symbol or instrument identifier.")
...
class DlForecastRequest(BaseModel):
    ...
    series_type: Optional[str] = Field(default=None, description="E.g. nav, price, return.")


class LlmSummariseRequest(BaseModel):
    text: str = Field(min_length=1, description="Report or document excerpt.")
