class ApiPaths:
    ...
    AI_RISK_VAR = f"{PREFIX}/ai/risk/var"
    AI_FRAUD_CHECK = f"{PREFIX}/ai/fraud/check"
    AI_SURVEILLANCE_TRADE = f"{PREFIX}/ai/surveillance/trade"
    AI_SENTIMENT = f"{PREFIX}/ai/sentiment"
    AI_IDENTITY_SCORE = f"{PREFIX}/ai/identity/score"
    AI_DL_FORECAST = f"{PREFIX}/ai/dl/forecast"
    AI_LLM_SUMMARISE = f"{PREFIX}/ai/llm/summarise"
    AI_OLLAMA_GENERATE = f"{PREFIX}/ai/ollama/generate"
    AI_HUGGINGFACE_SUMMARISE = f"{PREFIX}/ai/huggingface/summarise"
    AI_TF_FORECAST = f"{PREFIX}/ai/tf/forecast"
