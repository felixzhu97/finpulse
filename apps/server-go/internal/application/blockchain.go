...
type BlockchainLedger interface {
	GetLatestBlockForUpdate(ctx context.Context, tx pgx.Tx) (*domain.Block, error)
	GetLatestBlock(ctx context.Context) (*domain.Block, error)
	AppendBlock(ctx context.Context, tx pgx.Tx, block domain.Block, txs []domain.ChainTransaction) error
...
type BlockchainWallet interface {
	GetBalanceForUpdate(ctx context.Context, tx pgx.Tx, accountID, currency string) (*domain.WalletBalance, error)
	UpdateBalance(ctx context.Context, tx pgx.Tx, accountID, currency string, delta float64) (*domain.WalletBalance, error)
...
func NewBlockchainService(pool *pgxpool.Pool, ledger BlockchainLedger, wallet BlockchainWallet) *BlockchainService {
	return &BlockchainService{pool: pool, ledger: ledger, wallet: wallet}
}
