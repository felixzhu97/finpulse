...
import { accountsApi, instrumentsApi, ordersApi, tradesApi } from "@/src/api";
import type { Instrument, Order, Trade } from "@/src/types";
...
export function NewTradeDrawer({ visible, onClose, onSuccess }: NewTradeDrawerProps) {
  ...
  useEffect(() => {
    if (visible) {
      Promise.all([accountsApi.list(20, 0), instrumentsApi.list(50, 0)]).then(([accs, insts]) => {
        setAccounts(accs);
