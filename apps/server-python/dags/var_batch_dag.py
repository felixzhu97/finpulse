...
with DAG(
    ...
    trigger_batch_var = BashOperator(
        ...
        bash_command=f'curl -s -X POST "{BATCH_ENDPOINT}" -H "Content-Type: application/json" -d \'{DEFAULT_PAYLOAD}\' || echo "API not reachable; ensure server-python is running on host (e.g. port 8800)"',
    )
